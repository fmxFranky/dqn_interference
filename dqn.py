...
if __name__ == "__main__":
    ...
    parser.add_argument('--train-frequency',
                        type=int,
                        default=4,
...
class QValueVisualizationWrapper(gym.Wrapper):
    ...
    def render(self, mode="human"):
        if mode == "rgb_array":
            ...
            fig, ax = plt.subplots(figsize=(self.image_shape[1] / 100,
                                            self.image_shape[0] / 100),
...
ts = time.gmtime()
ts = time.strftime("%m%dT%H%M", ts)
experiment_name = f"{args.gym_id}-{args.exp_name}-{args.seed}-{ts}"
...
class QNetwork0(nn.Module):
    def __init__(self, frames=4):
        super(QNetwork0, self).__init__()
        self.cnn_encoder = nn.Sequential(Scale(1 / 255),
...
class QNetwork1(nn.Module):
    def __init__(self, frames=4):
        ...
        self.centers = nn.Embedding(8, 64)
        nn.init.orthogonal_(self.centers.weight)
        ...
        self.projector = nn.Sequential(nn.Linear(3136, 64), nn.ReLU())
        self.mlp = Linear0(512, env.action_space.n)
        self.mha_encoder = MHAEncoder(n_layers=3,
                                      n_heads=8,
                                      embedding_dim=64,
                                      input_dim=64,
    ...
    def forward(self, x):
        # x1.shape = [B, 64]
        x1 = self.projector(self.cnn_encoder(torch.Tensor(x).to(device)))
        # x2.shape = [B, 8, 64]
        x2 = self.centers(
            torch.arange(0, 8, dtype=torch.long,
                         device=device)[None].repeat(x.shape[0], 1))
        # x.shape = [B, 9, 64]
        x = torch.cat([x1[:, None, :], x2], dim=1)
        x = self.mha_encoder(x)
        # context_query.shape = [B, 1, 64]
        context_query = x[:, 0:1, :]
        # context_query.context_keys = [B, 8, 64]
        context_keys = x[:, 1:, :]
        # logits.shape = [B, 1, 8]
        logits = torch.matmul(context_query, context_keys.transpose(
            1, 2)) / np.sqrt(64)
        # probs.shape = [B, 8, 1]
        probs = F.softmax(logits, dim=-1).transpose(1, 2)
        # x.shape = [B, 8*64]
        z = context_keys.flatten(1, 2) * probs.repeat(1, 1, 64).flatten(1, 2)
        return self.mlp(z)
...
class QNetwork2(nn.Module):
    ...
    def forward(self, x):
        # x1.shape = [B, 64]
        x1 = self.projector(self.cnn_encoder(
            torch.Tensor(x).to(device)))[:, None, :]
        # x2.shape = [B, 8, 64]
        x2 = self.centers(
            torch.arange(0, 8, dtype=torch.long,
                         device=device)[None].repeat(x.shape[0], 1))
        # logits.shape = [B, 1, 8]
        logits = torch.matmul(x1, x2.transpose(1, 2)) / np.sqrt(64)
        # probs.shape = [B, 8, 1]
        probs = F.softmax(logits, dim=-1).transpose(1, 2)
        # x.shape = [B, 8*64]
        z = x2.flatten(1, 2) * probs.repeat(1, 1, 64).flatten(1, 2)
        return self.mlp(z)
...
for global_step in range(args.total_timesteps):
    ...
    if 'episode' in info.keys():
        ...
        writer.add_scalar("charts/episode_reward", info['episode']['r'],
                          global_step)
    ...
    if global_step > args.learning_starts and global_step % args.train_frequency == 0:
        ...
        with torch.no_grad():
            ...
            target_max = target_value.gather(
                1,
                torch.max(current_value, 1)[1].unsqueeze(1)).squeeze(1)
            td_target = torch.Tensor(s_rewards).to(
                device) + args.gamma * target_max * (
                    1 - torch.Tensor(s_dones).to(device))

        old_val = q_network.forward(s_obs).gather(
            1,
            torch.LongTensor(s_actions).view(-1, 1).to(device)).squeeze()
        ...
        optimizer.zero_grad()
        loss.backward()
        # for j in range(len(q_network.cnn_encoder)):
        #     if hasattr(q_network.cnn_encoder[j], 'weight'):
        #         print(q_network.cnn_encoder[j].weight.grad.abs().mean(),
        #               q_network.cnn_encoder[j].weight.grad.abs().max(),
        #               q_network.cnn_encoder[j].weight.grad.abs().min())
        # print(q_network.centers.weight.grad.mean(),
        #       q_network.centers.weight.grad.max(),
        #       q_network.centers.weight.grad.min(),
        #       q_network.centers.weight.grad.median())
        # input()
        nn.utils.clip_grad_norm_(list(q_network.parameters()),
                                 args.max_grad_norm)
        optimizer.step()
